...
// by Melnikov Anton
/*
  Включение двух светильников (основной+декоративный)

  Управление включением:
  1. Автоматически по датчикам движения и света
  2. Вручную пультом по RF-каналу

  Управление выключением:
  1. В автоматическом режиме по таймеру при отсутствии срабатывания датчика движения
  2. Вручную пультом по RF-каналу
...
#define main_led_pin   D5 // пин управления основной лентой
...
#define rfPin D2      // Data-pin RF приемника
...
//#define EEPROM_on // закомментировать, если EEPROM не используется
...
const char *mqtt_unique_client_id = "arduinoClient_KitcheLight113"; // Уникальный идентификатор
...
bool btn_pressed_A_mqtt = false;  // признак нажатия кнопки
bool btn_pressed_B_mqtt = false;  // признак нажатия кнопки
...
void loop() {
  ...
  */
  ...
  if ((btn_pressed_A) or (btn_pressed_A_mqtt)) // Если есть признак нажатия кнопки
  { btn_pressed_A_mqtt = false;
    ...
    btn_pressed_A = false; // снимаем признак нажатия кнопки
  ...
  // Вычисляем фактическую длительность предыдущего цикла // ---------------------------------------
